...
# ----------------------
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item.find(OrderItem.pluck(:item_id).uniq).pluck(:name).sort
    # OR:
    ordered_items_names = OrderItem.joins(:item).select(:items).distinct.order(:name).pluck(:name)
    # ---------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    .select("users.*, count(orders) as total_order_count")
    .group("users.id")
  ...
  it '28. returns a table of information for all users items' do
    ...
    .select("users.*, count(order_items) as total_item_count")
    .group(:id)
    .order(:name)
    # OR:
    custom_results = User.joins(orders: :order_items)
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    .select('users.name as user_name, orders.id as order_id, floor(orders.amount / count(order_items)) as avg_item_cost')
